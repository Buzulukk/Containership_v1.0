...
public class VisualizationRows implements ChangeListener {
    ...
    public void visualization(ContainerShip got_containerShip) {
        ...
        //frame
        frame = new JFrame("Visualization Rows");
        ...
        //slider
        slider = new JSlider(1, containerShip.maxRowsAmount, 1);
        ...
        changeModeButton.setText("Tiers");
        changeModeButton.setFocusable(false);
        changeModeButton.addActionListener(e -> {
            frame.setVisible(false);
            frame.dispose();
            VisualizationTiers visualizationTiers = new VisualizationTiers();
            visualizationTiers.visualization(containerShip);
        });
    ...
    private static void visualize(int row) {
        xContainerPosition = 10;
        frame.add(slider);
        frame.add(changeModeButton);
        for (int bay = 0; bay < containerShip.baysAmount; ++bay) {
            yContainerPosition = containerShip.maxTiersAmount * 50 + 100;
            for (int tier = containerShip.maxTiersAmount; tier >= 0; --tier) {
                ...
                if (tier == containerShip.tiersUnderMainDeck[1]) {
                    yContainerPosition += 10;
                }
                ...
                containerLabel.setBounds(xContainerPosition, yContainerPosition - (tier * 42), 17 * containersSize * (is40ft ? 2 : 1), 8 * containersSize);
                frame.add(containerLabel);
            }
            xContainerPosition += 17 * containersSize + 7;
